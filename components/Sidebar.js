...
function Sidebar() {
    ...
    return (
        ...
        <div className="flex items-center justify-between p-2 rounded-md bg-graylight">
            ...
            <div onClick={()=>auth.signOut()} className={showSignOutModal ? ' transition-all ease-in-out duration-200 cursor-pointer animate-pulse bg-graylight shadow-lg  p-2 z-50 rounded-md drop-shadow-lg w-max right-0 top-5 text-base absolute block':'hidden'}>Sign out</div>
            </div>
        </div>
            ...
            <div className="px-2 mt-16 ">
                ...
                <div className="my-8 overflow-y-scroll max-h-56">
                {
                    ...
                    {/* addfriend*/}
                    <form onSubmit={createChat} className={showModal ? "text-xs flex items-center border rounded-md shadow-lg sm:text-base border-graylight drop-shadow-lg":"hidden"}><input className="w-full p-2 bg-transparent focus:outline-none" type="text" placeholder="Enter your friend's email" value={email} onKeyDown={keyDown} onChange={(e)=>setEmail(e.target.value)} />
                    <button type="submit">
                        <PlusIcon className="h-5 mx-1"/>
                    </button >
                    </form>
                <div  onClick={()=>setShowModal(!showModal)} className="mt-4 text-sm ">
                    ...
                    <p className="mx-3">{!showModal ? 'Add Friends':'Cancel'}</p>
                    </div>
                    </div>
            </div>
        </div>
